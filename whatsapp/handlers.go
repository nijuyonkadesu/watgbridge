...
func (bc *bridgeContext) handleStickerMessage(v *events.Message) {
	...
	if stickerMsg.GetIsAnimated() || stickerMsg.GetIsAvatar() {
		...
		if webmBytes, err := utils.AnimatedWebpConvertToWebm(stickerBytes, v.Info.ID); err == nil {
			sentMsg, sendErr := bc.tgBot.SendSticker(bc.cfg.Telegram.TargetChatID,
			...
			if sendErr == nil {
				if sentMsg != nil && sentMsg.Sticker == nil {
					bc.logger.Warn("Telegram classified converted WEBM as a document",
						zap.String("whatsapp_message_id", v.Info.ID),
						zap.Int("bytes", len(webmBytes)),
					)
				}
				bc.savePair(sentMsg)
				return
			}
			bc.logger.Warn("failed to send converted WEBM as a Telegram sticker",
				zap.String("whatsapp_message_id", v.Info.ID),
				zap.Error(sendErr),
			)
		} else {
			bc.logger.Debug("failed to convert animated WhatsApp sticker to WEBM",
				zap.String("whatsapp_message_id", v.Info.ID),
				zap.Error(err),
			)
		}
